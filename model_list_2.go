/*
Mailchimp Marketing API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 3.0.55
Contact: apihelp@mailchimp.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mailchimpmarketingapi

import (
	"encoding/json"
)

// checks if the List2 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &List2{}

// List2 List settings for the campaign.
type List2 struct {
	// The unique list id.
	ListId *string `json:"list_id,omitempty"`
	// The status of the list used, namely if it's deleted or disabled.
	ListIsActive *bool `json:"list_is_active,omitempty"`
	// The name of the list.
	ListName *string `json:"list_name,omitempty"`
	// A description of the [segment](https://mailchimp.com/help/getting-started-with-groups/) used for the campaign. Formatted as a string marked up with HTML.
	SegmentText *string `json:"segment_text,omitempty"`
	// Count of the recipients on the associated list. Formatted as an integer.
	RecipientCount *int32 `json:"recipient_count,omitempty"`
	SegmentOpts *SegmentOptions1 `json:"segment_opts,omitempty"`
}

// NewList2 instantiates a new List2 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewList2() *List2 {
	this := List2{}
	return &this
}

// NewList2WithDefaults instantiates a new List2 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewList2WithDefaults() *List2 {
	this := List2{}
	return &this
}

// GetListId returns the ListId field value if set, zero value otherwise.
func (o *List2) GetListId() string {
	if o == nil || IsNil(o.ListId) {
		var ret string
		return ret
	}
	return *o.ListId
}

// GetListIdOk returns a tuple with the ListId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *List2) GetListIdOk() (*string, bool) {
	if o == nil || IsNil(o.ListId) {
		return nil, false
	}
	return o.ListId, true
}

// ContainsListId returns a boolean if a field has been set.
func (o *List2) ContainsListId() bool {
	if o != nil && !IsNil(o.ListId) {
		return true
	}

	return false
}

// SetListId gets a reference to the given string and assigns it to the ListId field.
func (o *List2) SetListId(v string) {
	o.ListId = &v
}

// GetListIsActive returns the ListIsActive field value if set, zero value otherwise.
func (o *List2) GetListIsActive() bool {
	if o == nil || IsNil(o.ListIsActive) {
		var ret bool
		return ret
	}
	return *o.ListIsActive
}

// GetListIsActiveOk returns a tuple with the ListIsActive field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *List2) GetListIsActiveOk() (*bool, bool) {
	if o == nil || IsNil(o.ListIsActive) {
		return nil, false
	}
	return o.ListIsActive, true
}

// ContainsListIsActive returns a boolean if a field has been set.
func (o *List2) ContainsListIsActive() bool {
	if o != nil && !IsNil(o.ListIsActive) {
		return true
	}

	return false
}

// SetListIsActive gets a reference to the given bool and assigns it to the ListIsActive field.
func (o *List2) SetListIsActive(v bool) {
	o.ListIsActive = &v
}

// GetListName returns the ListName field value if set, zero value otherwise.
func (o *List2) GetListName() string {
	if o == nil || IsNil(o.ListName) {
		var ret string
		return ret
	}
	return *o.ListName
}

// GetListNameOk returns a tuple with the ListName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *List2) GetListNameOk() (*string, bool) {
	if o == nil || IsNil(o.ListName) {
		return nil, false
	}
	return o.ListName, true
}

// ContainsListName returns a boolean if a field has been set.
func (o *List2) ContainsListName() bool {
	if o != nil && !IsNil(o.ListName) {
		return true
	}

	return false
}

// SetListName gets a reference to the given string and assigns it to the ListName field.
func (o *List2) SetListName(v string) {
	o.ListName = &v
}

// GetSegmentText returns the SegmentText field value if set, zero value otherwise.
func (o *List2) GetSegmentText() string {
	if o == nil || IsNil(o.SegmentText) {
		var ret string
		return ret
	}
	return *o.SegmentText
}

// GetSegmentTextOk returns a tuple with the SegmentText field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *List2) GetSegmentTextOk() (*string, bool) {
	if o == nil || IsNil(o.SegmentText) {
		return nil, false
	}
	return o.SegmentText, true
}

// ContainsSegmentText returns a boolean if a field has been set.
func (o *List2) ContainsSegmentText() bool {
	if o != nil && !IsNil(o.SegmentText) {
		return true
	}

	return false
}

// SetSegmentText gets a reference to the given string and assigns it to the SegmentText field.
func (o *List2) SetSegmentText(v string) {
	o.SegmentText = &v
}

// GetRecipientCount returns the RecipientCount field value if set, zero value otherwise.
func (o *List2) GetRecipientCount() int32 {
	if o == nil || IsNil(o.RecipientCount) {
		var ret int32
		return ret
	}
	return *o.RecipientCount
}

// GetRecipientCountOk returns a tuple with the RecipientCount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *List2) GetRecipientCountOk() (*int32, bool) {
	if o == nil || IsNil(o.RecipientCount) {
		return nil, false
	}
	return o.RecipientCount, true
}

// ContainsRecipientCount returns a boolean if a field has been set.
func (o *List2) ContainsRecipientCount() bool {
	if o != nil && !IsNil(o.RecipientCount) {
		return true
	}

	return false
}

// SetRecipientCount gets a reference to the given int32 and assigns it to the RecipientCount field.
func (o *List2) SetRecipientCount(v int32) {
	o.RecipientCount = &v
}

// GetSegmentOpts returns the SegmentOpts field value if set, zero value otherwise.
func (o *List2) GetSegmentOpts() SegmentOptions1 {
	if o == nil || IsNil(o.SegmentOpts) {
		var ret SegmentOptions1
		return ret
	}
	return *o.SegmentOpts
}

// GetSegmentOptsOk returns a tuple with the SegmentOpts field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *List2) GetSegmentOptsOk() (*SegmentOptions1, bool) {
	if o == nil || IsNil(o.SegmentOpts) {
		return nil, false
	}
	return o.SegmentOpts, true
}

// ContainsSegmentOpts returns a boolean if a field has been set.
func (o *List2) ContainsSegmentOpts() bool {
	if o != nil && !IsNil(o.SegmentOpts) {
		return true
	}

	return false
}

// SetSegmentOpts gets a reference to the given SegmentOptions1 and assigns it to the SegmentOpts field.
func (o *List2) SetSegmentOpts(v SegmentOptions1) {
	o.SegmentOpts = &v
}

func (o List2) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o List2) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ListId) {
		toSerialize["list_id"] = o.ListId
	}
	if !IsNil(o.ListIsActive) {
		toSerialize["list_is_active"] = o.ListIsActive
	}
	if !IsNil(o.ListName) {
		toSerialize["list_name"] = o.ListName
	}
	if !IsNil(o.SegmentText) {
		toSerialize["segment_text"] = o.SegmentText
	}
	if !IsNil(o.RecipientCount) {
		toSerialize["recipient_count"] = o.RecipientCount
	}
	if !IsNil(o.SegmentOpts) {
		toSerialize["segment_opts"] = o.SegmentOpts
	}
	return toSerialize, nil
}

type NullableList2 struct {
	value *List2
	isSet bool
}

func (v NullableList2) Get() *List2 {
	return v.value
}

func (v *NullableList2) Set(val *List2) {
	v.value = val
	v.isSet = true
}

func (v NullableList2) IsSet() bool {
	return v.isSet
}

func (v *NullableList2) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableList2(val *List2) *NullableList2 {
	return &NullableList2{value: val, isSet: true}
}

func (v NullableList2) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableList2) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


